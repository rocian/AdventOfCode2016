...
been using a one-time pad that you generate using a pre-agreed
...
def md5_input2(s):
    ...
    for i in range(0, 2017):
        st = hashlib.md5(st.encode('utf-8')).hexdigest()

    return(st)


def generate_md5(n, salt, index, std=1):
    lmd5 = []
    if std == 1:
        for i in range(0, n):
            lmd5.append(md5_input(salt + str(index + i)))

    else:
        for i in range(0, n):
            lmd5.append(md5_input2(salt + str(index + i)))

    return(lmd5)


def get_password(s, std):
    "Return index of keys as requested by part 1 method."
    alldone = 0
    index = 0
    triple = re.compile(r"(.)\1\1")

    lmd5 = generate_md5(2000, s, index, std)
    nmd5 = len(lmd5)

    for fmd5 in lmd5:
        grf = re.search(triple, fmd5)

        if grf:
            match = grf.group(0)[1] * 5
        ...
        if (nmd5 - index) < 1005:
            lmd5 += generate_md5(200, s, nmd5, std)
            nmd5 = len(lmd5)
        index += 1
        if alldone > 63:
            break
...
print("Day 14. Solution of part 1: {}".format(get_password(ststring, 1)))
print("Day 14. Solution of part 2: {}".format(get_password(ststring, 2)))
