...
def countsafe(row, maxrow):
    count = 0
    for i in range(maxrow):
        count += (len(row) - sum(row))
        nrow = [0] + row + [0]
        row = [nrow[i] ^ nrow[i + 2] for i in range(len(nrow) - 2)]

    return(count)


# The rules to calculate the new line can be summarized as
#
# called ABC the three interesting tiles, the four roules are
# (A, B, nC) + (nA, B, C) + (A, nB, nC) + (nA, nB, C)
# where , is an and, + an or and n stand for not
#
# the four rule summarize as
# A, nC + nA, C
# that is 
# A xor C

# so we need only a xor operation
# and is natural to convert the row in integer
# 0 safe, 1 trap
first = read_input()
row = [int(x == '^') for x in list(first)]

print("Day 18. Solution of part 1: {}".format(countsafe(row, 40)))
print("Day 18. Solution of part 2: {}".format(countsafe(row, 400000)))
